...
const DEFAULT_SSE_FUNCTION_NAME = "my-submissions-events-sse";

function getDefaultPath() {
  const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
  if (!supabaseUrl) return "/api/my-submissions-events";
...
export async function createMySubmissionsEventStream({
  userId,
  path = import.meta.env.VITE_MY_SUBMISSIONS_SSE_URL || getDefaultPath(),
  tokenFunctionName =
  ...
  if (!userId) {
    throw new Error("createMySubmissionsEventStream requires userId");
  }
  ...
  async function connectWithFreshToken() {
    ...
    try {
      ...
      nextSource.addEventListener("submission-upsert", (event) => {
        if (source !== nextSource || closed) return;
        const payload = parseEventData(event.data);
        if (payload) onSubmissionUpsert?.(payload);
      });

      nextSource.addEventListener("submission-remove", (event) => {
        if (source !== nextSource || closed) return;
        const payload = parseEventData(event.data);
        if (payload?.id) onSubmissionRemove?.(payload);
      });
